...
if (mysqli_num_rows($result) > 0) {
  while($row = mysqli_fetch_assoc($result)) {
    ...
    echo " <tr><td>" . $row["kode_mk"]. " </td><td> " . $row["matkul"]. "</td><>" . $row["sks"]. " </td><td>
    <a href='proses_delete.php?id=". $row["id"]."' <button type='button' class='btn btn-danger'>Delete</button></a>
    </td><td> <a href='form_edit_mata_kuliah.php?id=". $row["id"]."' <button type='button' class='btn btn-success'>Edit</button></a> </td></tr>";
  }
} else {
  echo "0 results";
}
