...
<main class="flex-shrink-0">
  <div class="container">
    <h1 class="mt-5">Halaman Tampil Data Krs</h1>
  ...
  <thead>
    <tr>
      <th scope="col">NIM</th>
      <th scope="col">NIP</th>
      <th scope="col">Kode Mata Kuliah</th></th>
...
if (mysqli_num_rows($result) > 0) {
  while($row = mysqli_fetch_assoc($result)) {
    ...
    echo " <tr><td>" . $row["nim"]. " </td><td> " . $row["nip"]. "</td><td>" . $row["kode_mk"]. "</td><td>". $row["absen"]."</td><td>". $row["uts"]."</td><td>". $row["uas"]."</td><td>
    <a href='proses_delete.php?id=". $row["id"]."' <button type='button' class='btn btn-danger'>Delete</button></a>
    </td><td> <a href='form_edit_krs.php?id=". $row["id"]."' <button type='button' class='btn btn-success'>Edit</button></a> </td></tr>";
  }
} else {
  echo "0 results";
}
