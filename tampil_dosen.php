...
<main class="flex-shrink-0">
  <div class="container">
    <h1 class="mt-5">Halaman Tampil Data Dosen</h1>
...
if (mysqli_num_rows($result) > 0) {
  while($row = mysqli_fetch_assoc($result)) {
    ...
    echo " <tr><td>" . $row["nip"]. " </td><td> " . $row["nama"]. "</td><td>" . $row["jeniskelamin"]. "</td><td>". $row["alamat"]."</td><td>". $row["notlp"]."</td><td>
    <a href='proses_delete.php?id=". $row["id"]."' <button type='button' class='btn btn-danger'>Delete</button></a>
    </td><td> <a href='form_edit_dosen.php?id=". $row["id"]."' <button type='button' class='btn btn-success'>Edit</button></a> </td></tr>";
  }
} else {
  echo "0 results";
}
